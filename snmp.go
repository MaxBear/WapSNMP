...
func (w WapSNMP) GetTable(oid Oid, maxRepetitions int) (map[string]interface{}, error) {
	result := make(map[string]interface{})
	lastOid := oid.Copy()
	for lastOid.Within(oid) {
		log.Printf("Sending GETBULK(%v, %d)\n", lastOid, maxRepetitions)
		results, err := w.GetBulk(lastOid, maxRepetitions)
		if err != nil {
